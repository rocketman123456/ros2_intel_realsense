...
namespace realsense
{
    ...
    {
        printDeviceInfo();
        //printSupportedStreamProfiles();
        // printActiveStreamProfiles();
        ...
        node_.set_on_parameters_set_callback(std::bind(
    ...
    void RealSenseBase::setupStream(const stream_index_pair &stream)
    {
        ...
        if (stream == ACCEL || stream == GYRO)
        {
            ...
            RCLCPP_INFO(node_.get_logger(), os.str());
            ...
            else
            {
                if(stream == ACCEL) 
                {
                    fps = node_.declare_parameter(os.str(), DEFAULT_ACCEL_FPS);
                }
                else if(stream == GYRO)
                {
                    fps = node_.declare_parameter(os.str(), DEFAULT_GYRO_FPS);
                }
            }
            imu_pub_.insert(
            ...
            imu_info_pub_.insert(
                std::pair<stream_index_pair, rclcpp::Publisher<realsense_msgs::msg::IMUInfo>::SharedPtr>(
                    stream,
                    node_.create_publisher<realsense_msgs::msg::IMUInfo>(INFO_TOPIC.at(stream), rclcpp::QoS(1))));
            if (enable == true)
            {
                enable_[stream] = true;
                cfg_.enable_stream(stream.first, stream.second, STREAM_FORMAT.at(stream.first), fps);
            }
        }
        else if (stream == POSE)
        {
            odom_pub_ = node_.create_publisher<nav_msgs::msg::Odometry>(
                SAMPLE_TOPIC.at(stream), rclcpp::QoS(1));
            if (enable == true)
            {
                enable_[stream] = true;
        ...
        else
        {
            ...
            if (stream == COLOR || stream == DEPTH || stream == INFRA1 || stream == INFRA2)
            {
                ...
                {
                    auto default_res = rclcpp::ParameterValue(DEFAULT_IMAGE_RESOLUTION);
                    res = node_.declare_parameter(os.str(),default_res).get<rclcpp::PARAMETER_INTEGER_ARRAY>();
                }
                os.str("");
                os << STREAM_NAME.at(stream.first) << stream.second << ".fps";
                RCLCPP_INFO(node_.get_logger(), os.str());
            ...
            camera_info_pub_.insert(
                std::pair<stream_index_pair, rclcpp::Publisher<sensor_msgs::msg::CameraInfo>::SharedPtr>(
                    stream,
                    node_.create_publisher<sensor_msgs::msg::CameraInfo>(INFO_TOPIC.at(stream), rclcpp::QoS(1))));
            if (enable == true)
            {
                enable_[stream] = true;
                cfg_.enable_stream(
        ...
        const rs2::stream_profile &base_profile)
    {
        ...
        if (type == RS2_STREAM_POSE)
        {
            ...
            composeTFMsgAndPublish(
                transform_ts, translation, Q, OPTICAL_FRAME_ID.at(type_index), base_frame_id_);
        }
        else
        {
            composeTFMsgAndPublish(
                transform_ts, translation, quaternion_optical, base_frame_id_, OPTICAL_FRAME_ID.at(type_index));
        }
    }
    ...
    Result RealSenseBase::changeResolution(const stream_index_pair &stream, const rclcpp::Parameter &param)
    {
        ...
        cfg_.enable_stream(
            stream.first, stream.second, res[0], res[1], STREAM_FORMAT.at(stream.first), stream_info_[stream].fps);
    ...
    Result RealSenseBase::changeMotionFPS(const stream_index_pair &stream, const rclcpp::Parameter &param)
    {
        auto result = Result();
        result.successful = true;
        if (param.get_type() != rclcpp::ParameterType::PARAMETER_INTEGER)
        ...
        int fps = param.as_int();
        RCLCPP_INFO(node_.get_logger(), "set motion fps");
        cfg_.enable_stream(stream.first, stream.second, STREAM_FORMAT.at(stream.first), fps);
        if (cfg_.can_resolve(pipeline_))
        {
            if (enable_[stream] == true)
